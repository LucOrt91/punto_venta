...
{
    public class ProviderRepositoty : GenericRepository<Provider>, IProvideRepository
    {
        public ProviderRepositoty(POSContext context) : base(context) { }


        public async Task<BaseEntityResponse<Provider>> ListProvider(BaseFilterRequest filters)
        {
            var response = new BaseEntityResponse<Provider>();

            var providers = GetEntityQuery(x =>x.AuditDeleteDate == null && x.AuditDeleteUser == null)
